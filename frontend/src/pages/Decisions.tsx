...
export default function Decisions() {
  ...
  useEffect(() => {
    api.getDecisions().then((data) => {
      const rows = (data || []) as RunEntry[];
      setItems(rows);
      // Auto-expand every top-level document group on initial load.
      setExpandedMRs(new Set(rows.filter((r) => !r.parent_id).map((r) => r.document_id)));
      setLoading(false);
                        ...
                        <React.Fragment key={entry.decision_id}>
                          ...
                          {isWfExpanded && children.map((child) => (
                            ...
                            >
                              ...
                              <TableCell className="text-xs whitespace-nowrap">
                                {(child.completed_at || child.started_at || "").substring(0, 19).replace("T", " ")}
                              </TableCell>
                            </TableRow>
                          ))}
                        </React.Fragment>
