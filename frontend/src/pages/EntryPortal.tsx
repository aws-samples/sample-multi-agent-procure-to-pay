...
import type { Requisition, Invoice } from "../types/p2p";
    ...
    >
      <CardContent className="p-5">
        <div className={`mb-3 inline-flex h-10 w-10 items-center justify-center rounded-lg ${c.bg}`}>
          {createElement(icon, { className: `h-5 w-5 ${c.text}` })}
        </div>
...
function RecentItems({
  ...
  return (
    <div className="flex flex-col">
      {display.map((item, i) => {
        const id = type === "pr" ? (item as Requisition).requisition_id : (item as Invoice).invoice_id;
...
export default function EntryPortal() {
  ...
  const [recentPRs, setRecentPRs] = useState<Requisition[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([loadMetrics, loadPRs, loadInvoices]).then(([m, prs, invs]) => {
      ...
      if (role === "requester" && user?.email) {
        filteredPRs = filteredPRs.filter((p) => p.requester === user.email);
      }
