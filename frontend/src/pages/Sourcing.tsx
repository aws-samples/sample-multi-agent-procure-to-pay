...
export default function Sourcing() {
  ...
  const prevSrcResult = useRef<any>(null);
  useEffect(() => {
    if (agent.result && agent.result !== prevSrcResult.current) {
      prevSrcResult.current = agent.result;
      setAgentResult(agent.result);
      setAgentMeta(agent.meta);
      setEvaluating(false);
    }
    if (agent.error && evaluating) {
                ...
                <div>
                  ...
                  <div className="rounded-lg border">
                    <Table>
                      ...
                      <TableBody>
                        {supplierEvaluations.map((e: any, i: number) => (
                          <TableRow key={i}>
                            ...
                            <TableCell>
                              <StatusBadge
                                status={
                                  e.total_score >= 70 ? "approved"
                                  : e.total_score >= 50 ? "warning"
                                  : "rejected"
                                }
                              >
                                {e.total_score}/100
