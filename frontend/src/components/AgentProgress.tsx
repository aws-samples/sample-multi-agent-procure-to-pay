...
export default function AgentStepList({
  ...
  if (macroSteps) {
    ...
    // Flat mode: dedupe identical labels, then assign status
    // (last one running if isRunning, rest done).
    ...
    steps = labels.map((label, i) => ({
      label,
      status: isRunning && i === labels.length - 1 ? "running" as const : "done" as const,
    }));
  }
 ...
 */
// eslint-disable-next-line react-refresh/only-export-components -- step-derivation helper co-located with AgentStepList; consumed by Requisitions page. Splitting into a separate module is a larger refactor with no runtime benefit.
